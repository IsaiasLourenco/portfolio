  <!-- Facts Start -->
  <div class="container-fluid facts py-5 pt-lg-0">
    <div class="container py-5 pt-lg-0">
      <div class="row gx-0">
        <div class="col-lg-4 wow zoomIn" data-wow-delay="0.1s">
          <div class="bg-primary shadow d-flex align-items-center justify-content-center p-4" style="height: 150px">
            <div class="bg-light d-flex align-items-center justify-content-center rounded mb-2" style="width: 60px; height: 60px">
              <i class="bi bi-people-fill text-secondary"></i>
            </div>
            <div class="ps-4">
              <h5 class="text-dark mb-0">Clientes Satisfeitos</h5>
              <h1 class="text-dark mb-0" data-toggle="counter-up">12345</h1>
            </div>
          </div>
        </div>
        <div class="col-lg-4 wow zoomIn" data-wow-delay="0.3s">
          <div class="bg-light shadow d-flex align-items-center justify-content-center p-4" style="height: 150px">
            <div class="bg-secondary d-flex align-items-center justify-content-center rounded mb-2" style="width: 60px; height: 60px">
              <i class="bi bi-check-circle-fill text-primary"></i>
            </div>
            <div class="ps-4">
              <h5 class="text-dark mb-0">Projetos Prontos</h5>
              <h1 class="mb-0" data-toggle="counter-up">21345</h1>
            </div>
          </div>
        </div>
        <div class="col-lg-4 wow zoomIn" data-wow-delay="0.6s">
          <div class="bg-primary shadow d-flex align-items-center justify-content-center p-4" style="height: 150px">
            <div class="bg-light d-flex align-items-center justify-content-center rounded mb-2" style="width: 60px; height: 60px">
              <i class="bi bi-award-fill text-secondary"></i>
            </div>
            <div class="ps-4">
              <h5 class="text-dark mb-0">Reconhecimento e Realizações</h5>
              <h1 class="text-dark mb-0" data-toggle="counter-up">31245</h1>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- Facts Start -->